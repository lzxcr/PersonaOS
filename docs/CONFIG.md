...
| 主菜单项 | 功能 |
|---|---|
| 文本模型 | 从模型池选择激活的文本模型；`n` 输入 `provider/model` 新建 |
| 多模态模型 | 同上，针对多模态池 |
| 子代理档位池 | `Tab` 切换 cheap/balanced/strong 三档，`Enter` 选入/移出模型 |
| 供应商和模型 | 供应商列表 CRUD（`a` 添加、`d` 删除、`s` 设为当前）；`Enter` 进入字段总览，再 `Enter` 内联编辑 |
| 插件配置 | 14 个插件启用开关（空格）；`Enter` 进入字段总览（全参数编辑）；`api_quota` 进入多账号管理 |
| 自定义提示词 | 人格列表激活（Enter）；`n` 新建、`r` 重命名、`d` 删除（自动清理作用域目录） |
| 接入平台 | 平台列表（空格 启用/禁用）；`Enter` 进入字段总览（连接/权限/限流等全量字段），再 `Enter` 内联编辑 |
| 全局参数设置 | 14 项工具/技能/显示参数总览（含当前值），`Enter` 内联编辑 |

#### 编辑交互（统一规范）

所有字段编辑页（供应商/插件/接入平台/全局参数）使用统一的**内联编辑**模式：

- `Enter` 进入编辑：当前行变为输入框（下划线高亮），显示当前值可修改
- `Char`/`Backspace` 修改；`Enter` 应用并跳到下一字段（连续编辑）
- `←`/`→` 切换布尔值（true↔false）和备选项（choices 循环）
- `↑↓`/`jk` 切换字段（自动加载当前值）；`n` 同 `↓`
- `Esc` 退出编辑返回总览；校验失败红字提示
