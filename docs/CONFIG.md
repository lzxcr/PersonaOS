...
| 供应商和模型 | 供应商列表 CRUD：`Enter` 编辑字段（id/名称/URL/key/模型）、`a` 模板添加、`d` 删除、`s` 设为当前 |
| 插件配置 | 14 个插件启用开关（空格）；`Enter` 进入详情编辑全部字段；`api_quota` 插件进入多账号管理（Tab 切换 DeepSeek/OpenRouter，a 添加账号） |
| 自定义提示词 | 人格列表激活（Enter）；`n` 新建、`r` 重命名、`d` 删除（自动清理作用域目录） |
| 接入平台 | 平台启用开关（空格）；`Enter` 编辑全量字段（QQ：连接/权限/中间消息/用户识别/会话限流共 13 项，含空格切换 bool） |
| 全局参数设置 | 14 项工具/技能/显示参数，带取值范围校验 |
