...
const WORKER_ENV: &str = "POS_INTERNAL_RENDERER_WORKER";
...
const RENDERER_FONTS_ENV: &str = "POS_RENDERER_FONTS_DIR";
...
impl WorkerProcess {
    async fn spawn() -> Result<Self> {
        let executable = std::env::current_exe().context("locating the PersonaOS executable")?;
...
fn renderer_fonts_dir() -> Result<PathBuf> {
    ...
    candidates.push(PathBuf::from("/usr/share/pos/fonts"));
    if let Ok(executable) = std::env::current_exe() {
        if let Some(prefix) = executable.parent().and_then(std::path::Path::parent) {
            candidates.push(prefix.join("share/pos/fonts"));
        }
    ...
    let searched = candidates
        ...
        .join(", ");
    bail!(
        "renderer font is missing; install {CJK_FONT_FILE} in /usr/share/pos/fonts or set {RENDERER_FONTS_ENV} (searched: {searched})"
    )
}
