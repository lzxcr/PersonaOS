...
impl DeleteConfirmations {
    fn issue(
        ...
        let phrase = format!("确认删除 PersonaOS 历史 范围={scope} 模式={mode} {token}");
        let now = Instant::now();
...
async fn delete(
    ...
    if !effective_admin(&context) {
        bail!("only a configured PersonaOS platform administrator may delete history");
    }
...
fn require_cross_conversation_access(
    ...
    if !effective_admin(context) {
        bail!("only a PersonaOS platform administrator may access another conversation's history");
    }
    Ok(())
}
