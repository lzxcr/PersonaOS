...
pub(super) fn store_for_paths(paths: &PersonaPaths) -> HistoryStore {
...
fn history_db_path(paths: &PersonaPaths) -> PathBuf {
...
impl MessageHistoryPlugin {
    ...
    async fn record_inbound(paths: &PersonaPaths, event: &PlatformInboundEvent) -> Result<()> {
    ...
    async fn record_outbound(
        ...
        let message_id = receipt
            ...
            .first()
            .cloned()
            .unwrap_or_else(|| format!("pos-{timestamp}-{:08x}", rand::random::<u32>()));
        let sender_name = context
            .bot_display_name()
            .await
            .unwrap_or_else(|_| "PersonaOS".to_string());
            ...
            store_for_paths(&context.paths)
                .record_message(NewHistoryMessage {
                    ...
                    message_id: if message_id.trim().is_empty() {
                        format!("pos-{timestamp}-{:08x}", rand::random::<u32>())
                    } else {
