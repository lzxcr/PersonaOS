    ...
    fn register_tools(
        ...
        registry.register(
            ToolSpec::new(
                ...
                "Directly manage PersonaOS's QQ administrators, private-chat whitelist, and group-chat whitelist. Call this when the current PersonaOS administrator asks to grant, revoke, or list access. Grant and revoke take effect immediately. The Rust host sends the final QQ result, so do not call send_message_to_user and do not add another acknowledgement.",
                json!({
                    "type": "object",
                    "properties": {
                        "operation": {
...
fn execute_access(arguments: &Value, context: &PlatformTurnContext) -> Result<String> {
    if context.conversation.platform != ONEBOT_PLATFORM || !effective_admin(context) {
        bail!("platform access management requires a current PersonaOS administrator");
    }
    ...
    context.with_current_config(|config| {
        ...
        match result {
            PlatformAccessMutationResult::Unauthorized => {
                bail!("the requesting user is no longer a PersonaOS administrator")
            }
...
fn format_access_list(
    ...
    let (config, grants) = context.with_current_config(|config| -> Result<_> {
        ...
        let grants = context
            ...
            .context("the requesting user is no longer a PersonaOS administrator")?;
        Ok((qq.clone(), grants))
    })?;
