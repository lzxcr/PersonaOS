...
fn edit_custom_prompts(
    ...
    loop {
        let persona = if config.prompt.active_persona.trim().is_empty() {
            match crate::prompts::default_builtin_persona() {
                Ok(default) => {
                    if crate::i18n::is_zh() {
                        default.display_name.to_string()
                    } else {
                        default.display_name_en.to_string()
                    }
                }
                Err(_) => "PersonaOS".to_string(),
            }
        } else {
...
fn edit_personas(stdout: &mut io::Stdout, paths: &PersonaPaths, config: &mut AppConfig) -> Result<()> {
...
impl PersonaMenuTarget {
    ...
    fn custom_offset(&self) -> usize {
        self.builtin_offset() + crate::prompts::builtin_personas().len()
    }

    fn is_builtin(&self, name: &str, config: &AppConfig) -> bool {
        let is_default_builtin = crate::prompts::default_builtin_persona()
        ...
        match self {
            Self::Global => {
                if is_default_builtin {
                    ...
                    config.prompt.active_persona.trim()
                        == crate::prompts::builtin_persona_file(name)
                }
            }
            Self::Platform(persona) => persona.builtin_name() == Some(name),
        }
    }
            ...
            Self::Global => {
                ...
                if active.is_empty()
                    || crate::prompts::is_builtin_canonical(
                        active.strip_suffix(".md").unwrap_or(active),
                    )
                {
                    None
                } else {
                    Some(config.prompt.active_persona.as_str())
                }
            }
    ...
    fn activate_builtin(&mut self, config: &mut AppConfig, name: &str) {
        match self {
            Self::Global => {
            ...
            Self::Platform(persona) => {
                *persona = PlatformPersonaOverride::Builtin {
                    name: name.to_string(),
                }
            }
        }
...
fn manage_personas(
    ...
    let builtin_offset = target.builtin_offset();
    ...
    loop {
        ...
        for bp in builtins {
            let label = crate::i18n::is_zh()
                .then_some(bp.display_name)
                .unwrap_or(bp.display_name_en);
            options.push(format!(
                "{}{label}",
                if target.is_builtin(bp.canonical_name, config) {
                    "* "
                } else {
                    "  "
                }
        ...
        match read_key()? {
            ...
            KeyCode::Tab => {
                ...
                } else if selected >= builtin_offset && selected < custom_offset {
                    let bp_index = selected - builtin_offset;
                    if let Some(bp) = builtins.get(bp_index) {
                        target.activate_builtin(config, bp.canonical_name);
                    }
...
fn remove_persona_scope(paths: &PersonaPaths, config: &AppConfig, name: &str) -> Result<()> {
...
fn list_identities(paths: &PersonaPaths, config: &AppConfig) -> Result<Vec<String>> {
    list_markdown_files(&config.identities_dir_path(paths))
}

fn read_identity(paths: &PersonaPaths, config: &AppConfig, name: &str) -> Result<String> {
    let path = config.identity_path(paths, name);
    if path.exists() {
...
fn write_identity(paths: &PersonaPaths, config: &AppConfig, name: &str, content: &str) -> Result<()> {
    let path = config.identity_path(paths, name);
...
fn list_personas(paths: &PersonaPaths, config: &AppConfig) -> Result<Vec<String>> {
...
fn read_persona(paths: &PersonaPaths, config: &AppConfig, name: &str) -> Result<String> {
    let path = config.persona_path(paths, name);
    if path.exists() {
        Ok(std::fs::read_to_string(path)?)
    } else {
...
fn write_persona(paths: &PersonaPaths, config: &AppConfig, name: &str, content: &str) -> Result<()> {
    let path = config.persona_path(paths, name);
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent)?;
    }
...
fn auto_configure_model_tags(paths: &PersonaPaths, provider: &mut ProviderConfig, model: &str) {
...
fn edit_qq(stdout: &mut io::Stdout, paths: &PersonaPaths, config: &mut AppConfig) -> Result<()> {
    let mut selected = 0usize;
    loop {
...
fn platform_persona_summary(persona: &PlatformPersonaOverride) -> String {
    match persona {
        ...
        PlatformPersonaOverride::Builtin { name } => {
            if let Some(bp) = crate::prompts::builtin_persona(name) {
                if crate::i18n::is_zh() {
...
mod tests {
    ...
    fn real_context_batch_parsers_are_line_based_and_deduplicated() {
        ...
        assert_eq!(mappings[0].nickname, "PersonaOS");
        assert_eq!(mappings[0].user_id, 123);
        assert!(parse_real_context_identity_lines("PersonaOS\t123\nPersonaOS\t456").is_err());
        assert!(parse_real_context_identity_lines("PersonaOS 123").is_err());

        assert_eq!(
            parse_real_context_string_lines("晚安\n 晚安 \nPersonaOS", 128).unwrap(),
            vec!["晚安", "PersonaOS"]
        );
    }
    ...
    #[test]
    fn qq_conversation_persona_summary_distinguishes_inheritance_and_builtins() {
        assert_eq!(
            platform_persona_summary(&PlatformPersonaOverride::Inherit),
            t("inherit current persona", "继承当前人格")
        );
        assert_eq!(
            platform_persona_summary(&PlatformPersonaOverride::Builtin {
                name: "test".to_string()
            }),
            "Test"
        );
    ...
    fn qq_persona_menu_target_isolated_from_global_persona_and_tracks_renames() {
        ...
        let n_builtins = crate::prompts::builtin_personas().len();
        assert_eq!(target.custom_offset(), 1 + n_builtins);
        ...
        target.activate_builtin(&mut config, "alice");
        assert!(target.is_builtin("alice", &config));
        assert!(!target.is_builtin("test", &config));
        assert_eq!(config.prompt.active_persona, "Global.md");

        target.activate_builtin(&mut config, "test");
        assert!(target.is_builtin("test", &config));
        assert_eq!(config.prompt.active_persona, "Global.md");
        target.activate_inherit();
        assert!(matches!(
    ...
    fn global_persona_menu_target_preserves_activation_behavior() {
        ...
        let n_builtins = crate::prompts::builtin_personas().len();
        assert_eq!(target.custom_offset(), n_builtins);
        assert!(target.is_builtin("test", &config));
        ...
        assert_eq!(
            config.prompt.active_persona,
            crate::prompts::builtin_persona_file("alice")
        );
        assert!(!target.is_builtin("test", &config));
    }
