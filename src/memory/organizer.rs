...
impl MemoryOrganizer {
    pub(crate) fn spawn() -> Result<Self> {
        ...
        let join = std::thread::Builder::new()
            .name("pos-memory-organizer".to_string())
            .spawn(move || {
...
impl MemoryOrganizerHandle {
    pub(crate) fn wake(&self, config: AppConfig, paths: PersonaPaths, state_store: StateStore) {
...
mod tests {
    ...
    fn test_paths(temp: &tempfile::TempDir) -> PersonaPaths {
        PersonaPaths {
