...
pub fn alarms_file(paths: &PersonaPaths) -> PathBuf {
    paths.state_dir.join("alarms.json")
}

pub fn alarm_log_file(paths: &PersonaPaths) -> PathBuf {
...
pub fn load(paths: &PersonaPaths) -> Result<Vec<AlarmRecord>> {
...
pub fn save(paths: &PersonaPaths, records: &[AlarmRecord]) -> Result<()> {
...
pub fn upsert(paths: &PersonaPaths, record: AlarmRecord) -> Result<()> {
...
pub fn update_status(paths: &PersonaPaths, id: &str, status: AlarmStatus) -> Result<()> {
    let mut records = load(paths)?;
...
pub fn remove(paths: &PersonaPaths, id: &str) -> Result<Option<AlarmRecord>> {
    let mut records = load(paths)?;
...
pub fn cleanup_dead(paths: &PersonaPaths) -> Result<Vec<AlarmRecord>> {
