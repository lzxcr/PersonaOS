...
impl StateStore {
    pub fn new(paths: &PersonaPaths) -> Result<Self> {
    ...
    pub fn init_files(&self) -> Result<()> {
        ...
        if !self.profile_file().exists() {
            std::fs::write(self.profile_file(), "# PersonaOS Profile\n\n")?;
        }
        Ok(())
    }
...
mod tests {
    ...
    fn turn_lifecycle() {
        ...
        let store = StateStore::new(&PersonaPaths {
        ...
        assert!(!temp.path().join("state/pos.log").exists());
    ...
    fn session_crud_switching_and_persona_adoption() {
        ...
        let listed = store.list_sessions("pos", false).unwrap();
        ...
        store
            ...
            .unwrap();
        assert_eq!(store.list_sessions("pos", false).unwrap().len(), 1);
        assert_eq!(store.list_sessions("pos", true).unwrap().len(), 2);
        ...
        assert_eq!(store.list_sessions("pos", true).unwrap().len(), 1);
    ...
    fn persona_reset_clears_active_local_and_onebot_contexts_only() {
        ...
        let local = store.create_session("pos", "local", "user", None).unwrap();
        let archived = store
            .create_session("pos", "archived", "user", None)
            .unwrap();
        store
        ...
        let other_persona = store
            ...
            .unwrap();
        let qq = store.create_session("pos", "qq", "user", None).unwrap();
        ...
        store
            ...
            .unwrap();
        let subagent = store
            .create_session("pos", "child", "subagent", Some(&local.session_id))
            .unwrap();
        ...
        let targets = store.persona_reset_session_ids("pos", "onebot").unwrap();
        ...
        let cleared = store.reset_persona_contexts("pos", "onebot").unwrap();
        ...
        assert_eq!(
            store.platform_session_bindings("pos", "onebot").unwrap()[0].session_id,
            qq.session_id
        );
    }
    ...
    fn platform_bindings_survive_rename_and_isolate_personas() {
        let (_temp, store) = test_store();
        let pos_session = store
            .create_session("pos", "old display name", "user", None)
            .unwrap();
        let other_session = store
            .create_session("other", "another display name", "user", None)
            .unwrap();
        let pos_key = platform_binding_key("20000", None, "pos");
        let other_key = platform_binding_key("20000", None, "other");

        store
            .bind_platform_session(&pos_key, &pos_session.session_id)
            .unwrap();
        store
            .bind_platform_session(&other_key, &other_session.session_id)
            .unwrap();
        store
            .rename_session(&pos_session.session_id, "new display name")
            .unwrap();

        assert_eq!(
            store.find_platform_session_binding(&pos_key).unwrap(),
            Some(pos_session.session_id.clone())
        );
        // `None` and an empty participant are the same database identity.
        let empty_participant_key = platform_binding_key("20000", Some(""), "pos");
        assert_eq!(
            store
        ...
        store.delete_session(&pos_session.session_id).unwrap();
        assert_eq!(
            store.find_platform_session_binding(&pos_key).unwrap(),
            None
        );
    ...
    fn local_session_listing_excludes_platform_owned_history() {
        let (_temp, store) = test_store();
        let local = store
            .create_session("pos", "shared name", "user", None)
            .unwrap();
        let platform = store
            .create_session("pos", "shared name", "user", None)
            .unwrap();
        let key = platform_binding_key("20000", None, "pos");
        store
            .bind_platform_session(&key, &platform.session_id)
            .unwrap();

        let all_ids = store
            .list_sessions("pos", false)
            .unwrap()
            .into_iter()
        ...
        assert_eq!(
            store
                .find_local_session_by_name("pos", "SHARED NAME")
                .unwrap()
                .unwrap()
                .session_id,
            local.session_id
    ...
    fn platform_binding_overwrite_and_conflict_are_atomic() {
        let (_temp, store) = test_store();
        let session_a = store.create_session("pos", "a", "user", None).unwrap();
        let session_b = store.create_session("pos", "b", "user", None).unwrap();
        let session_c = store.create_session("pos", "c", "user", None).unwrap();
        let key_a = platform_binding_key("group-a", None, "pos");
        let key_b = platform_binding_key("group-b", None, "pos");
    ...
    fn concurrent_platform_bind_rejects_session_sharing() {
        ...
        let session = store
            .create_session("pos", "shared target", "user", None)
            .unwrap();
        ...
        let handles = [store.clone(), second_store]
            ...
            .map(|(store, conversation_id)| {
                ...
                let key = platform_binding_key(conversation_id, None, "pos");
    ...
    fn concurrent_platform_claim_converges_on_one_session() {
        ...
        let session_a = store.create_session("pos", "a", "user", None).unwrap();
        let session_b = store.create_session("pos", "b", "user", None).unwrap();
        let key = platform_binding_key("same-group", None, "pos");
        let barrier = Arc::new(std::sync::Barrier::new(3));
        let handles = [
    ...
    fn platform_session_creation_is_bound_atomically() {
        let (_temp, store) = test_store();
        let key = platform_binding_key("atomic-group", None, "pos");
    ...
    fn platform_plugin_json_is_shared_across_personas_and_supports_deletion() {
        ...
        let pos_session = store.create_session("pos", "pos", "user", None).unwrap();
        let other_session = store
            .create_session("other", "other", "user", None)
            .unwrap();
        let pos_store = store.pinned(&pos_session.session_id);
        let other_store = store.pinned(&other_session.session_id);
        let from_pos: Option<Vec<String>> =
            pos_store.plugin_get_json(&scope, "recent_images").unwrap();
        let from_other: Option<Vec<String>> = other_store
            .plugin_get_json(&scope, "recent_images")
            .unwrap();
        assert_eq!(from_pos, Some(replacement.clone()));
    ...
    fn subagent_audit_sessions_are_hidden_and_expire() {
        ...
        let audit = store
            .create_session("pos", "探索代码库", "subagent", Some(&parent))
            .unwrap();
    ...
    fn clearing_pinned_session_content_is_isolated_and_preserves_usage_and_binding() {
        ...
        let target_record = store
            .create_session("pos", "qq:10000:private:42", "user", None)
            .unwrap();
        ...
        target
            ...
            .unwrap();
        let binding = platform_binding_key("42", None, "pos");
        store
