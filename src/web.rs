...
const AUTH_COOKIE: &str = "pos_session";
...
const POS_LOGO: &[u8] = include_bytes!("../pics/pos-logo.png");
const POS_WALLPAPER: &[u8] = include_bytes!("../pics/pos-wallpaper.png");
...
pub async fn run(paths: PersonaPaths, args: WebArgs) -> Result<()> {
    ...
    let listener = match tokio::net::TcpListener::bind(SocketAddr::new(bind_ip, args.port)).await {
                ...
                && error.kind() == std::io::ErrorKind::AddrInUse =>
        {
            tracing::warn!(
                ...
                "{}",
                t(
                    "PersonaOS WebUI default port is occupied; selecting an ephemeral port",
                    "PersonaOS WebUI 默认端口已被占用；将选择临时端口"
                )
            );
            tokio::net::TcpListener::bind(SocketAddr::new(bind_ip, 0))
                .await
                .context("binding PersonaOS WebUI to an ephemeral fallback port")?
        }
        Err(error) => {
            return Err(error)
                .with_context(|| format!("binding PersonaOS WebUI to 0.0.0.0:{}", args.port));
        }
    };
    ...
    drop(ipc_lease);
    serve_result.context("serving PersonaOS WebUI")?;
    actor_result
}
...
fn start_ipc_server(
    ...
    let lease = ipc::acquire_web_core(&state.paths)
        .context("another PersonaOS core is already running or starting")?;
    let socket_path = state.paths.ipc_socket();
    let listener = tokio::net::UnixListener::bind(&socket_path)
        .with_context(|| format!("binding PersonaOS IPC socket at {}", socket_path.display()))?;
            ...
            let (stream, _) = match listener.accept().await {
                ...
                Err(error) => {
                    tracing::warn!(
                        error = %error,
                        "{}",
                        t("PersonaOS IPC listener stopped", "PersonaOS IPC 监听器已停止")
                    );
                    break;
                }
            };
            ...
            tokio::spawn(async move {
                ...
                if let Err(error) = handle_ipc_connection(connection_state, stream).await {
                    tracing::debug!(
                        ...
                        "{}",
                        t(
                            "PersonaOS IPC connection closed with an error",
                            "PersonaOS IPC 连接因错误关闭"
                        )
                    );
                }
            });
...
async fn handle_ipc_connection(
    ...
    .await
    .context("timed out waiting for a PersonaOS IPC request")??
    else {
        return Ok(());
    };
    ...
    match request.command {
        ...
        IpcCommand::ReloadConfig => {
                ...
                .is_err()
            {
                ...
                ipc::send(
                    &mut stream,
                    &IpcFrame::error("PersonaOS core worker is unavailable"),
                )
                .await?;
                return Ok(());
            }
            match receiver.await {
                ...
                Err(_) => {
                    ...
                    ipc::send(
                        &mut stream,
                        &IpcFrame::error("PersonaOS core stopped while reloading configuration"),
                    )
                    .await?
                }
            }
        ...
        IpcCommand::ResetConversation { all, target } => {
            ...
            if all {
                ...
                match reset_platform_persona_state(&state, &config).await {
                    ...
                    Err(PlatformPersonaResetError::Unavailable) => {
                        anyhow::bail!("PersonaOS core worker is unavailable");
                    }
            ...
            match receiver.await {
                ...
                Err(_) => {
                    release_admin(&state.manager);
                    anyhow::bail!("PersonaOS core stopped while resetting the conversation");
                }
            }
        }
        IpcCommand::Undo { target } => {
            ...
            match receiver.await {
                ...
                Err(_) => {
                    release_admin(&state.manager);
                    anyhow::bail!("PersonaOS core stopped while undoing the conversation");
                }
            }
        }
        IpcCommand::Pop { target, turn_ids } => {
            ...
            match receiver.await {
                ...
                Err(_) => {
                    release_admin(&state.manager);
                    anyhow::bail!("PersonaOS core stopped while popping the conversation");
                }
            }
        }
        IpcCommand::Compact { target } => {
            ...
            match receiver.await {
                ...
                Err(_) => {
                    release_admin(&state.manager);
                    anyhow::bail!("PersonaOS core stopped while compacting the conversation");
                }
            }
        }
...
async fn switch_session_via_actor(
    ...
    {
        release_admin(&state.manager);
        return Err("PersonaOS core worker is unavailable".to_string());
    }
    match receiver.await {
        Ok(Ok(())) => Ok(()),
        ...
        Err(_) => {
            release_admin(&state.manager);
            Err("PersonaOS core stopped while switching sessions".to_string())
        }
    }
}
...
async fn switch_session_via_actor_reserved(
    ...
    match receiver.await {
        Ok(Ok(())) => Ok(()),
        Ok(Err(AdminFailure::Invalid(message) | AdminFailure::Internal(message))) => Err(message),
        Err(_) => Err("PersonaOS core stopped while switching sessions".to_string()),
    }
}
...
async fn handle_ipc_turn(
        ...
        .is_err()
    {
        ...
        ipc::send(stream, &IpcFrame::error("PersonaOS core worker is unavailable")).await?;
        return Ok(());
    }
    ...
    loop {
        ...
        if record.kind == "resync_required" {
            ipc::send(
                stream,
                &IpcFrame::error("PersonaOS core event history was exhausted; the turn was cancelled"),
            )
            .await?;
            break;
        }
...
fn router(state: DaemonState) -> Router {
    Router::new()
        ...
        .route("/assets/pos-logo.png", get(logo_asset))
        .route("/assets/pos-wallpaper.png", get(wallpaper_asset))
        ...
        // Backward-compatible endpoint used by earlier PersonaOS releases.
        .route(
            "/onebot/v11/ws",
...
async fn logo_asset() -> Response {
    binary_asset(POS_LOGO, "image/png")
}

async fn wallpaper_asset() -> Response {
    binary_asset(POS_WALLPAPER, "image/png")
}
...
async fn redo_turn(
    ...
    let (cancel_tx, cancel_rx) = tokio::sync::watch::channel(false);
    {
        ...
        if manager.admin_busy || manager.session_has_runs(&session_id) {
            return Err(ApiError::new(
                StatusCode::CONFLICT,
                "PersonaOS is busy in this conversation",
            ));
        }
        manager.active_runs.insert(
            run_id.clone(),
...
fn spawn_actor(
    ...
    let join = std::thread::Builder::new()
        .name("pos-daemon-core".to_string())
        .spawn(move || {
    ...
    let setup = (|| -> Result<(Agent, AgentTurnControl)> {
        ...
        if local_webui && matches!(mode, AgentMode::Normal | AgentMode::Plan) {
            ...
            turn_system_context.push(format!(
                "<artifact-workspace>\n{manifest}\n使用 read_artifact 和 apply_artifact_patch 按文件名操作已有 Artifact；不要用 glob 搜索托管目录，也不要猜测 ~/.pos 路径。\n</artifact-workspace>"
            ));
            runtime_system_context.push(
                "<artifact-policy>\n\
                你正在 PersonaOS WebUI 中工作，并且拥有 Artifact 展示工具。\n\
...
fn build_actor_agent(config: &AppConfig, paths: &PersonaPaths, state: &StateStore) -> Result<Agent> {
    let mut agent = build_session_agent(config, paths, state)?;
    agent.prepare_for_turn()?;
    Ok(agent)
}

fn build_session_agent(config: &AppConfig, paths: &PersonaPaths, state: &StateStore) -> Result<Agent> {
    crate::models_cache::ensure_active_metadata(paths, config);
...
fn persona_identity(config: &AppConfig, prompts: &PromptDocuments) -> PersonaIdentity {
    ...
    // Default or non-default builtin persona (no user file on disk).
    let maybe_builtin = if active.is_empty() {
        crate::prompts::default_builtin_persona().ok()
    } else {
        let canonical = active.strip_suffix(".md").unwrap_or(active);
        let has_document = prompts
            .personas
            .iter()
            .any(|document| document.name == active);
        if !has_document {
            crate::prompts::builtin_persona(canonical)
        } else {
            None
        }
    };
    if let Some(bp) = maybe_builtin {
        return PersonaIdentity {
            name: if crate::i18n::is_zh() {
                bp.display_name.to_string()
            } else {
                bp.display_name_en.to_string()
            },
            avatar_url: bp.avatar_asset.map(str::to_string),
            board_image_url: bp.board_asset.map(str::to_string),
            board_title: bp.board_title.unwrap_or(DEFAULT_BOARD_TITLE).to_string(),
            board_subtitle: bp
                .board_subtitle
                .unwrap_or(DEFAULT_BOARD_SUBTITLE)
                .to_string(),
            starter_prompts: if bp.starter_prompts.is_empty() {
                DEFAULT_STARTER_PROMPTS.map(str::to_string).to_vec()
            } else {
                bp.starter_prompts.iter().map(|s| s.to_string()).collect()
            },
        };
    }
    // No builtin persona registered and no custom persona active: fall back to
    // the neutral PersonaOS brand identity so the UI has something to show.
    if active.is_empty() {
        return PersonaIdentity {
            name: "PersonaOS".to_string(),
            avatar_url: Some("/assets/pos-logo.png".to_string()),
            board_image_url: Some("/assets/pos-wallpaper.png".to_string()),
...
fn resolve_persona_asset_path(paths: &PersonaPaths, value: &str) -> Option<PathBuf> {
...
fn managed_persona_asset_path(paths: &PersonaPaths, value: &str) -> Option<PathBuf> {
    let value = value.trim();
...
fn validate_managed_persona_asset_file(paths: &PersonaPaths, path: &FilePath) -> Result<()> {
...
fn validate_prompt_documents(
    ...
    if !config.prompt.active_persona.trim().is_empty()
        ...
        && !crate::prompts::is_builtin_canonical(
            config
                .prompt
                .active_persona
                .trim()
                .strip_suffix(".md")
                .unwrap_or(config.prompt.active_persona.trim()),
        )
    {
        return Err(ApiError::new(
            StatusCode::BAD_REQUEST,
...
fn read_prompt_documents(config: &AppConfig, paths: &PersonaPaths) -> Result<PromptDocuments> {
    Ok(PromptDocuments {
        ...
        for (change_index, (old_name, new_name)) in changes.iter().enumerate() {
            ...
            for (scope_index, original) in old_paths.into_iter().enumerate() {
                ...
                let staged = parent.join(format!(
                    ".pos-web-scope-{}-{change_index}-{scope_index}",
                    random_token(10)
                ));
...
mod tests {
    ...
    fn test_paths(root: &FilePath) -> PersonaPaths {
        PersonaPaths {
    ...
    async fn platform_session_reset_is_serialized_per_target_session() {
        ...
        let target = state
            .state_store
            .create_session("pos", "qq target", "user", None)
            .unwrap();
        let other = state
            .state_store
            .create_session("pos", "other", "user", None)
            .unwrap();
    ...
    fn startup_repairs_a_platform_owned_current_session() {
        let temp = tempfile::tempdir().unwrap();
        let store = StateStore::new(&test_paths(temp.path())).unwrap();
        store.adopt_sessions_for_persona("pos").unwrap();
        let qq_session = store
            .create_session("pos", "QQ group 20000", "user", None)
            .unwrap();
        store
            .bind_platform_session(
                &PlatformSessionBindingKey {
        ...
        ensure_local_current_session(&store, "pos").unwrap();
    ...
    fn cookie_parser_matches_an_exact_cookie_name() {
        ...
        headers.insert(
            COOKIE,
            HeaderValue::from_static("other=1; pos_session=secret-token; suffix=2"),
        );
