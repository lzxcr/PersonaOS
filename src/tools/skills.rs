...
fn stable_catalog(config: &AppConfig, paths: &PersonaPaths) -> Result<(Vec<SkillEntry>, [u8; 32])> {
    for _ in 0..3 {
...
pub fn register_authoring(registry: &mut ToolRegistry, config: AppConfig, paths: PersonaPaths) {
...
fn register_load_skill(
    ...
    let description = format!(
        ...
        t(
            "Use this tool before applying a skill or using any scripts/resources from that skill. Skill allowed-tools metadata never grants PersonaOS permissions.",
            "应用 skill 或使用其中的脚本/资源前，必须先加载该 skill。Skill 的 allowed-tools 元数据不会授予 PersonaOS 权限。",
        ),
...
fn register_create_skill(registry: &mut ToolRegistry, config: AppConfig, paths: PersonaPaths) {
    registry.register(
        ToolSpec::new(
            "create_skill",
            t(
                "Create a hidden draft for a new PersonaOS skill. Use the returned absolute skill_dir and skill_file with apply_patch, then call publish_skill. This never overwrites an existing skill.",
                "为新的 PersonaOS skill 创建隐藏草稿。使用返回的绝对 skill_dir 和 skill_file 配合 apply_patch 编辑，随后调用 publish_skill。此操作不会覆盖已有 skill。",
            ),
            json!({
                "type": "object",
                "properties": {
...
fn register_update_skill(registry: &mut ToolRegistry, config: AppConfig, paths: PersonaPaths) {
    registry.register(
        ToolSpec::new(
            "update_skill",
            t(
...
fn register_delete_skill(registry: &mut ToolRegistry, config: AppConfig, paths: PersonaPaths) {
    registry.register(
        ToolSpec::new(
            "delete_skill",
            t(
...
fn register_publish_skill(registry: &mut ToolRegistry, paths: PersonaPaths) {
    registry.register(
        ToolSpec::new(
            "publish_skill",
            t(
                "Validate and atomically publish a PersonaOS skill draft. Create drafts never overwrite; update drafts use revision checks. Scripts remain resources and are not registered as tools.",
                "校验并原子发布 PersonaOS skill 草稿。创建草稿绝不覆盖；更新草稿执行版本检查。scripts 仍是资源，不会注册为工具。",
            ),
            json!({
                "type": "object",
                "properties": {
...
fn register_list_skill_drafts(registry: &mut ToolRegistry, paths: PersonaPaths) {
    registry.register(
        ToolSpec::new(
            "list_skill_drafts",
            t(
                "List retained PersonaOS skill drafts. Drafts with no changes for 30 days are removed before listing.",
                "列出保留的 PersonaOS skill 草稿。列出前会清理 30 天未修改的草稿。",
            ),
...
fn load_skill(args: Value, config: &AppConfig, paths: &PersonaPaths) -> Result<String> {
...
fn create_skill(args: Value, config: &AppConfig, paths: &PersonaPaths) -> Result<String> {
    let name = required_string(&args, "name")?;
...
fn update_skill(args: Value, config: &AppConfig, paths: &PersonaPaths) -> Result<String> {
    let name = required_string(&args, "name")?;
...
fn publish_skill(args: Value, paths: &PersonaPaths) -> Result<String> {
...
fn delete_skill(args: Value, config: &AppConfig, paths: &PersonaPaths) -> Result<String> {
    let name = required_string(&args, "name")?;
    let scope_value = required_string(&args, "scope")?;
    let scope = SkillScope::parse(Some(&scope_value))?;
...
mod tests {
    ...
    fn loaded_skill_exposes_standard_metadata_without_granting_permissions() {
        ...
        assert!(loaded.contains("<compatibility>PersonaOS</compatibility>"));
        assert!(loaded
