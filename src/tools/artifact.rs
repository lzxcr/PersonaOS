...
pub fn register_webui(registry: &mut ToolRegistry, paths: &PersonaPaths, session_id: &str) {
...
pub fn managed_manifest(paths: &PersonaPaths, session_id: &str) -> Result<String> {
    let root = paths.data_dir.join("artifacts");
...
fn register_create(registry: &mut ToolRegistry, root: PathBuf, session_id: &str) {
    ...
    registry.register(ToolSpec::new_with_progress(
        "create_artifact",
        t(
            "Create or update a completed deliverable in PersonaOS's managed Artifact workspace and display it in the WebUI. Use this for reports, documents, standalone code, HTML, JSON, CSV, and other files the user should inspect. Do not use it for routine project source edits.",
            "在 PersonaOS 托管的 Artifact 工作区创建或更新已完成的交付物，并展示到 WebUI。适用于报告、文档、独立代码、HTML、JSON、CSV 等需要用户查看的文件；不要用于普通项目源码修改。",
        ),
        json!({
            "type": "object",
            "properties": {
