...
pub fn register(registry: &mut ToolRegistry, paths: PersonaPaths) {
    ...
    registry.register(ToolSpec::new(
        "set_alarm",
        t(
            "Set a local alarm or countdown. Accepts duration like 30s, 10m, 1h 30m, or a time like 14:30. The alarm runs in a background PersonaOS process and uses PersonaOS's embedded sound.",
            "设置本地闹钟或倒计时。支持 30s、10m、1h 30m 或 14:30。闹钟在后台 PersonaOS 进程运行，并使用 PersonaOS 内置声音。",
        ),
        json!({
            "type": "object",
            "properties": {
                ...
                "audio_file": { "type": "string", "description": t("Optional local .wav or .mp3 audio file to play instead of PersonaOS's built-in alarm sound.", "可选本地 .wav 或 .mp3 音频文件，用它替代 PersonaOS 内置闹钟音。") }
            },
            "required": ["time"],
...
async fn set_alarm(args: Value, paths: PersonaPaths) -> Result<String> {
    let time = args
        .get("time")
...
async fn list_alarms(paths: PersonaPaths) -> Result<String> {
...
async fn cancel_alarm(args: Value, paths: PersonaPaths) -> Result<String> {
    let id = args
        .get("id")
        .and_then(Value::as_str)
        .unwrap_or_default()
