...
pub fn register(registry: &mut ToolRegistry, paths: &PersonaPaths) {
    let dirs = [
    ...
    match scan_scripts(&dirs) {
        Ok(scan) => {
            ...
            if let Err(error) = registry.replace_script_tools(specs, scan.unregistered) {
                tracing::warn!(error = %error, "failed to register PersonaOS script tools");
            }
        }
        Err(error) => {
            tracing::warn!(error = %error, "failed to scan PersonaOS script directories during tool registration");
        }
    }
    register_script_tools(registry, paths.scripts_dir.clone());
}

pub fn rescan_scripts(registry: &mut ToolRegistry, paths: &PersonaPaths) {
    let dirs = [
        paths.system_scripts_dir.as_path(),
        paths.scripts_dir.as_path(),
    ];
    let scan = match scan_scripts(&dirs) {
        Ok(scan) => scan,
        Err(error) => {
            tracing::warn!(error = %error, "failed to rescan PersonaOS script directories");
            return;
        }
    };
    let specs = script_specs(&scan.entries, &paths.scripts_dir);
    if let Err(error) = registry.replace_script_tools(specs, scan.unregistered) {
        tracing::warn!(error = %error, "failed to replace PersonaOS script tools");
    }
}
...
fn resolve_script_path(path_str: &str, scripts_dir: &Path) -> PathBuf {
    ...
    if p.is_absolute() {
        ...
        if let Some(base) = directories::BaseDirs::new() {
            let legacy = base.config_dir().join("pos/scripts");
            if let Ok(relative) = p.strip_prefix(&legacy) {
                return scripts_dir.join(relative);
            }
        }
    ...
    registry.register(ToolSpec::new(
        ...
        json!({
            ...
            "properties": {
                ...
                "description": {
                    "type": "string",
                    "description": t("Optional tool description override. If omitted, PersonaOS reads the script header lines `Description:`/`description:` or `描述：` and sends only one localized description to the AI.", "可选的工具描述覆盖。省略时 PersonaOS 会读取脚本头部的 `Description:`/`description:` 或 `描述：`，并只向 AI 提供一条本地化描述。")
                },
                "path": {
                    "type": "string",
