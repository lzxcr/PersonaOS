...
pub fn clear_aur_review_state(paths: &PersonaPaths) -> anyhow::Result<()> {
    package_advisor::clear_aur_review_state(paths)
}

pub fn builtin_registry(config: &AppConfig, paths: &PersonaPaths) -> ToolRegistry {
...
pub fn webui_artifact_manifest(paths: &PersonaPaths, session_id: &str) -> anyhow::Result<String> {
...
pub fn readonly_registry(config: &AppConfig, paths: &PersonaPaths) -> ToolRegistry {
    let mut registry = ToolRegistry::new();
...
pub fn chat_registry(config: &AppConfig, paths: &PersonaPaths) -> ToolRegistry {
    let mut registry = ToolRegistry::new();
    web::register_fetch(&mut registry);
    if config.plugins.web.enabled {
        web::register(&mut registry, config.plugins.web.clone());
...
pub fn restricted_platform_registry(config: &AppConfig, paths: &PersonaPaths) -> ToolRegistry {
    let mut registry = ToolRegistry::new();
    web::register_fetch(&mut registry);
    weather::register(&mut registry);
    caniplayonlinux_query::register(&mut registry);
...
mod tests {
    ...
    fn test_paths(root: &std::path::Path) -> PersonaPaths {
        PersonaPaths {
            ...
            fish_hook_file: root.join("config/fish/conf.d/pos.fish"),
...
mod tier_schema_probe {
    ...
    fn task_definition_includes_tier() {
        ...
        let paths = crate::paths::PersonaPaths::new().unwrap();
