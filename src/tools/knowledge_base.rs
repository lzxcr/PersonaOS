...
pub fn register(registry: &mut ToolRegistry, config: AppConfig, paths: PersonaPaths) {
...
pub fn register_readonly(registry: &mut ToolRegistry, config: AppConfig, paths: PersonaPaths) {
    registry.register(ToolSpec::new(
...
impl KnowledgeBase {
    pub fn new(config: AppConfig, paths: PersonaPaths) -> Result<Self> {
    ...
    fn is_non_default_builtin(&self) -> bool {
        let active = self.config.prompt.active_persona.trim();
        if active.is_empty() {
            return false;
        }
        let canonical = active.strip_suffix(".md").unwrap_or(active);
        crate::prompts::builtin_persona(canonical)
            .is_some_and(|bp| !bp.is_default)
    }
    ...
    fn read_file_existing(
        ...
        if self.is_non_default_builtin() && rel.starts_with("default-kb/") {
            ...
            .open(&lock_path)
        {
            ...
            Err(_) => {
                if !quiet {
                    ...
                    println!(
                        "if no pos reindex process is running, remove the stale lock file and retry"
                    );
                }
                return Ok(0);
            }
...
async fn tool_search_readonly(args: Value, config: AppConfig, paths: PersonaPaths) -> Result<String> {
...
async fn tool_find_readonly(args: Value, config: AppConfig, paths: PersonaPaths) -> Result<String> {
    ensure_enabled(&config)?;
    let query = args
        .get("file_name_query")
        .and_then(Value::as_str)
...
async fn tool_read_readonly(args: Value, config: AppConfig, paths: PersonaPaths) -> Result<String> {
    ensure_enabled(&config)?;
    let name = args
        .get("file_name")
        .and_then(Value::as_str)
...
async fn tool_upload(args: Value, config: AppConfig, paths: PersonaPaths) -> Result<String> {
    ensure_enabled(&config)?;
    if !config.plugins.knowledge_base.upload_tool_enabled {
...
async fn tool_edit(args: Value, config: AppConfig, paths: PersonaPaths) -> Result<String> {
    ensure_enabled(&config)?;
    let name = args
        .get("file_name")
        .and_then(Value::as_str)
    ...
    Ok(json!({
        ...
        "warning": if name.starts_with("default-kb/") { Some("default-kb files may be overwritten by pos update-default-kb") } else { None::<&str> },
    })
    .to_string())
}

async fn tool_remove(args: Value, config: AppConfig, paths: PersonaPaths) -> Result<String> {
    ensure_enabled(&config)?;
    let name = args
        .get("file_name")
        .and_then(Value::as_str)
    ...
    Ok(json!({
        "ok": true,
        "path": rel,
        "warning": if name.starts_with("default-kb/") { Some("default-kb files may be restored by pos update-default-kb") } else { None::<&str> },
    })
    .to_string())
}
...
fn kb_root(config: &KnowledgeBasePluginConfig, paths: &PersonaPaths) -> PathBuf {
...
mod tests {
    ...
    fn test_paths(root: &Path) -> PersonaPaths {
        PersonaPaths {
            ...
            fish_hook_file: root.join("fish/conf.d/pos.fish"),
