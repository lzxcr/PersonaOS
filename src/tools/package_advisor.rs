...
async fn review_aur_package(args: Value, paths: PersonaPaths) -> Result<String> {
...
async fn install_aur_package(args: Value, paths: PersonaPaths) -> Result<String> {
    let package = required(&args, "package")?;
...
async fn install_with_makepkg_fallback(package: &str, paths: &PersonaPaths) -> Result<Value> {
    let metadata = fetch_aur_metadata(package).await?;
...
pub fn clear_aur_review_state(paths: &PersonaPaths) -> Result<()> {
...
fn review_state_for_package(paths: &PersonaPaths, package: &str) -> Result<Option<Value>> {
    Ok(load_review_state(paths)?.get(package).cloned())
}

fn record_install_confirmation(paths: &PersonaPaths, package: &str) -> Result<()> {
    let mut state = load_review_state(paths)?;
...
fn load_review_state(paths: &PersonaPaths) -> Result<Value> {
    let path = aur_review_state_path(paths);
    if !path.exists() {
        return Ok(json!({}));
    }
...
fn aur_review_state_path(paths: &PersonaPaths) -> PathBuf {
...
mod tests {
    ...
    fn test_paths(state_dir: PathBuf) -> PersonaPaths {
        PersonaPaths {
