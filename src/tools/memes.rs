...
const BUILTIN_MEMES_DIR: &str = "/usr/share/pos/memes";
...
pub fn register_chat(registry: &mut ToolRegistry, config: AppConfig, paths: PersonaPaths) {
    if !config.plugins.memes.enabled {
        return;
    }
...
fn register_search_and_show(registry: &mut ToolRegistry, config: AppConfig, paths: PersonaPaths) {
...
async fn search_meme(args: Value, config: &AppConfig, paths: &PersonaPaths) -> Result<String> {
...
async fn add_meme(args: Value, config: &AppConfig, paths: &PersonaPaths) -> Result<String> {
    let library = selected_library(&args, config);
...
async fn update_meme(args: Value, config: &AppConfig, paths: &PersonaPaths) -> Result<String> {
    let library = selected_library(&args, config);
    let library_lock = library_lock(&library);
    let _guard = library_lock.lock().await;
    let id = required_str(&args, "id")?;
...
async fn delete_meme(args: Value, config: &AppConfig, paths: &PersonaPaths) -> Result<String> {
    let library = selected_library(&args, config);
    let library_lock = library_lock(&library);
    let _guard = library_lock.lock().await;
...
fn load_library(paths: &PersonaPaths, library: &str) -> Result<Vec<LoadedMeme>> {
...
fn find_meme(paths: &PersonaPaths, library: &str, id: &str) -> Result<Option<LoadedMeme>> {
...
pub(crate) fn meme_ref_exists(paths: &PersonaPaths, meme: &MemeRef) -> Result<bool> {
...
fn builtin_library_dir(library: &str) -> PathBuf {
    if let Some(path) = std::env::var_os("POS_MEMES_DIR") {
...
fn user_library_dir(paths: &PersonaPaths, library: &str) -> PathBuf {
...
mod tests {
    ...
    fn sanitize_library_keeps_simple_names() {
        assert_eq!(sanitize_library("PersonaOS"), "personaos");
    ...
    fn current_library_follows_persona_mapping() {
        ...
        assert_eq!(current_persona_library(&config), "test");
