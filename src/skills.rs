...
pub fn discover(config: &AppConfig, paths: &PersonaPaths) -> Result<Vec<SkillEntry>> {
...
pub fn catalog_fingerprint(config: &AppConfig, paths: &PersonaPaths) -> Result<[u8; 32]> {
...
pub fn load(name: &str, config: &AppConfig, paths: &PersonaPaths) -> Result<LoadedSkill> {
    let name = name.trim();
    if name.is_empty() {
...
pub fn publish_draft(paths: &PersonaPaths, draft_id: &str) -> Result<PublishedSkill> {
...
pub fn list_drafts(paths: &PersonaPaths) -> Result<Vec<SkillDraft>> {
    prune_expired_drafts(paths)?;
...
pub fn prune_expired_drafts(paths: &PersonaPaths) -> Result<usize> {
    let _lease = acquire_publish_lock(paths)?;
    prune_expired_drafts_unlocked(paths)
}

fn prune_expired_drafts_unlocked(paths: &PersonaPaths) -> Result<usize> {
    let root = paths.skill_drafts_dir();
    if !root.is_dir() {
        return Ok(0);
    }
...
pub fn is_generated_skill(raw: &str) -> bool {
    parse_skill_metadata(raw, None)
        .ok()
        .and_then(|metadata| metadata.metadata.get("pos.generated").cloned())
        .is_some_and(|value| value.eq_ignore_ascii_case("true"))
        || raw.contains("generated_by: personaos")
        || raw.contains("Auto-learned method from assistant conversation")
        || raw.contains("Auto-learned method from PersonaOS conversation")
}
...
fn skill_roots(config: &AppConfig, paths: &PersonaPaths) -> Vec<(PathBuf, SkillSource)> {
    vec![
        (
...
fn create_empty_draft(paths: &PersonaPaths, manifest: &DraftManifest) -> Result<PathBuf> {
    let root = paths.skill_drafts_dir();
...
fn write_draft_manifest(paths: &PersonaPaths, manifest: &DraftManifest) -> Result<()> {
...
fn draft_public(paths: &PersonaPaths, manifest: &DraftManifest) -> Result<SkillDraft> {
    let skill_dir = paths
        .skill_drafts_dir()
        .join(&manifest.id)
...
fn read_manifest(paths: &PersonaPaths, draft_id: &str) -> Result<DraftManifest> {
    validate_draft_id(draft_id)?;
...
fn acquire_publish_lock(paths: &PersonaPaths) -> Result<PublishLease> {
    let root = paths.skill_drafts_dir();
    create_private_dir(&root)?;
...
mod tests {
    ...
    fn parses_standard_frontmatter_fields() {
        ...
        assert_eq!(metadata.compatibility.as_deref(), Some("PersonaOS"));
