...
fn cache_file(paths: &crate::paths::PersonaPaths) -> PathBuf {
...
fn fetch_and_cache(path: &PathBuf) -> Result<HashMap<String, HashMap<String, ModelInfo>>> {
    ...
    let text = client
        .get(API_URL)
        .header("User-Agent", "Mozilla/5.0 PersonaOS/0.1")
        .send()?
        .error_for_status()?
        .text()?;
...
pub fn try_load(paths: &crate::paths::PersonaPaths) {
...
pub fn try_load_active(paths: &crate::paths::PersonaPaths, config: &crate::config::AppConfig) {
    let path = cache_file(paths);
    let data = load_from_disk(&path).ok();
...
pub fn spawn_background_refresh(paths: crate::paths::PersonaPaths) {
    let path = cache_file(&paths);
...
pub fn spawn_background_refresh_active(
    paths: crate::paths::PersonaPaths,
...
pub fn ensure_active_metadata(paths: &crate::paths::PersonaPaths, config: &crate::config::AppConfig) {
    if !is_loaded() {
...
fn fetch_provider_context_windows(
    ...
    let mut request = client
        ...
        .header("User-Agent", "pos-model-metadata");
    if !api_key.is_empty() {
...
pub fn refresh_blocking(paths: &crate::paths::PersonaPaths) -> Result<()> {
    let _refresh = refresh_lock().lock().unwrap();
    if is_loaded() {
        return Ok(());
    }
