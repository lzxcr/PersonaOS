...
pub const ADMIN_BUSY_MESSAGE: &str = "PersonaOS is busy with another operation";
...
pub const BUILD_ID: &str = env!("POS_BUILD_ID");

/// Access URLs for the WebUI: loopback plus every local IPv4 address.
/// Shared between the daemon (startup banner) and the CLI (`pos web` /
/// `--status` output).
...
pub fn acquire_direct_core(paths: &PersonaPaths) -> Result<DirectCoreLease> {
    prepare_runtime_dir(paths)?;
    acquire_direct_core_at(paths.ipc_lock())
}

pub fn acquire_web_core(paths: &PersonaPaths) -> Result<WebCoreLease> {
    prepare_runtime_dir(paths)?;
...
fn prepare_runtime_dir(paths: &PersonaPaths) -> Result<()> {
...
fn acquire_lock(lock_path: PathBuf) -> Result<File> {
    ...
    if result != 0 {
        bail!("PersonaOS Web core is starting or temporarily unavailable");
    }
    Ok(lock_file)
}
...
pub async fn connect(path: &Path) -> Result<UnixStream> {
    UnixStream::connect(path)
        .await
        .with_context(|| format!("connecting to PersonaOS core at {}", path.display()))
}

pub async fn daemon_info(paths: &PersonaPaths) -> Option<DaemonInfo> {
...
pub fn stage_managed_web_password(paths: &PersonaPaths, password: &str) -> Result<PathBuf> {
...
pub fn stage_web_password_file(paths: &PersonaPaths, source: &Path) -> Result<PathBuf> {
...
fn try_load_daemon_launch_config(paths: &PersonaPaths) -> Result<Option<DaemonLaunchConfig>> {
...
fn load_daemon_launch_config(paths: &PersonaPaths) -> Result<DaemonLaunchConfig> {
...
fn save_daemon_launch_config(paths: &PersonaPaths, config: &DaemonLaunchConfig) -> Result<()> {
    let path = paths.daemon_launch_state_file();
...
fn commit_daemon_launch_config(paths: &PersonaPaths, config: &DaemonLaunchConfig) -> Result<()> {
...
fn abandon_daemon_launch_candidate(paths: &PersonaPaths, config: &DaemonLaunchConfig) {
...
fn remove_managed_password(paths: &PersonaPaths, path: &Path) {
...
fn remap_managed_password(
    ...
    previous: &PersonaPaths,
    current: &PersonaPaths,
) {
...
fn write_private_state(path: &Path, contents: &[u8]) -> Result<()> {
    let parent = path.parent().context("PersonaOS state file has no parent")?;
...
fn finish_private_state_commit(parent: &Path, directory_sync: std::io::Result<()>) -> Result<()> {
    if let Err(error) = directory_sync {
        tracing::warn!(
            ...
            error = %error,
            "PersonaOS state file was committed, but syncing its parent directory failed"
        );
    }
    Ok(())
}
...
#[cfg(target_os = "linux")]
fn recover_legacy_daemon_launch(paths: &PersonaPaths, pid: u32) -> Result<DaemonLaunchConfig> {
    let cmdline = std::fs::read(format!("/proc/{pid}/cmdline"))
        .context("reading legacy PersonaOS daemon arguments")?;
    let cwd = std::fs::read_link(format!("/proc/{pid}/cwd"))
        .context("reading legacy PersonaOS daemon working directory")?;
...
fn recover_legacy_daemon_launch(_paths: &PersonaPaths, _pid: u32) -> Result<DaemonLaunchConfig> {
...
pub fn discard_daemon_launch_candidate(paths: &PersonaPaths, config: &DaemonLaunchConfig) {
...
pub async fn ensure_daemon(
    ...
    if current.is_none() {
        ...
        active_paths = match PersonaPaths::new().context("refreshing PersonaOS paths before daemon startup")
        {
            Ok(paths) => paths,
            Err(error) => {
    ...
    if let Some(info) = current {
        ...
        active_paths = match PersonaPaths::new().context("refreshing PersonaOS paths after daemon shutdown")
        {
            Ok(paths) => paths,
            Err(error) => {
                if let Some(launch) = &pending_launch {
    ...
    loop {
        ...
        match child.try_wait().context("checking PersonaOS daemon process") {
            Ok(Some(status)) => {
                abandon_daemon_launch_candidate(&active_paths, &launch);
                bail!("PersonaOS daemon exited before becoming ready ({status})");
            }
            Ok(None) => {}
            Err(error) => {
                let _ = child.kill();
        ...
        if tokio::time::Instant::now() >= deadline {
            ...
            let _ = child.wait();
            abandon_daemon_launch_candidate(&active_paths, &launch);
            bail!("PersonaOS daemon did not become ready within 8 seconds");
        }
...
async fn restart_stale_daemon(paths: &PersonaPaths, info: &DaemonInfo) -> Result<()> {
    shutdown_daemon(paths, info)
        .await
        .context("waiting for the outdated PersonaOS daemon to stop")
}

pub async fn shutdown_daemon(paths: &PersonaPaths, info: &DaemonInfo) -> Result<()> {
...
pub async fn wait_for_daemon_exit(process: DaemonProcessIdentity, timeout: Duration) -> Result<()> {
    ...
    loop {
        ...
        if tokio::time::Instant::now() >= deadline {
            bail!(
                "PersonaOS daemon PID {} did not stop within {} seconds",
                process.pid,
                timeout.as_secs()
            );
        }
...
fn acquire_starter(paths: &PersonaPaths) -> Result<StarterLease> {
    prepare_runtime_dir(paths)?;
    let lock_file = OpenOptions::new()
        .create(true)
        .truncate(false)
...
fn start_daemon_process(
    ...
    let executable =
        std::env::current_exe().context("resolving the PersonaOS executable to spawn the daemon")?;
    ...
    command.spawn().context("starting PersonaOS daemon")
}
...
mod tests {
    ...
    fn daemon_process_prefers_the_default_web_port_unless_overridden() {
        let mut default = std::process::Command::new("pos");
        ...
        let mut overridden = std::process::Command::new("pos");
    ...
    fn test_paths(root: &Path) -> PersonaPaths {
        PersonaPaths {
    ...
    fn bare_launch_restores_the_saved_port_and_password_file() {
        ...
        let mut command = std::process::Command::new("pos");
    ...
    fn legacy_inline_password_and_port_are_recovered_into_managed_state() {
        let temp = tempfile::tempdir().unwrap();
        let paths = test_paths(temp.path());
        let cmdline = b"/usr/bin/pos\0__daemon\0--port\09412\0--password=legacy-secret\0";
    ...
    fn legacy_relative_password_file_is_copied_from_the_daemon_cwd() {
        ...
        let cmdline = b"pos\0__daemon\0--port=9500\0--password-file\0external-password\0";
