...
const BASH_BEGIN_MARKER: &str = "# >>> pos bash hook >>>";
const BASH_END_MARKER: &str = "# <<< pos bash hook <<<";
const ZSH_BEGIN_MARKER: &str = "# >>> pos zsh hook >>>";
const ZSH_END_MARKER: &str = "# <<< pos zsh hook <<<";
...
/// Refreshes only hook blocks installed by an older PersonaOS layout. It never
...
fn replace_marked_block(
    ...
    let Some(begin_index) = existing.find(begin) else {
        if existing.contains(end) {
            bail!("shell startup file contains a PersonaOS end marker without its begin marker");
        }
        return Ok(None);
    };
    let Some(end_relative) = existing[begin_index..].find(end) else {
        bail!("shell startup file contains an incomplete PersonaOS hook block");
    };
...
mod tests {
    ...
    fn classifies_commands_as_shell() {
        ...
        assert!(is_shell_command("./target/release/pos hi", "fish"));
