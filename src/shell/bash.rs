...
const BEGIN_MARKER: &str = "# >>> pos bash hook >>>";
const END_MARKER: &str = "# <<< pos bash hook <<<";
...
pub fn hook() -> &'static str {
    ...
    pos --shell-intercept --shell bash -- "$@" 2>/dev/null
    return 127
}
"#
}
...
mod tests {
    ...
    fn installing_again_refreshes_an_existing_hook_path() {
        ...
        std::fs::write(
            &rc_path,
            format!("before\n{BEGIN_MARKER}\nsource '/old/pos-hook.sh'\n{END_MARKER}\nafter\n"),
        )
        .unwrap();
        let hook = temp.path().join("new pos-hook.sh");
        ...
        assert!(updated.contains("new pos-hook.sh"));
        assert!(!updated.contains("/old/pos-hook.sh"));
