...
const BEGIN_MARKER: &str = "# >>> pos zsh hook >>>";
const END_MARKER: &str = "# <<< pos zsh hook <<<";
...
pub fn hook() -> &'static str {
    ...
    pos --shell-intercept --shell zsh -- "$@" 2>/dev/null
    return 127
}
"#
}
...
mod tests {
    ...
    fn zsh_hook_does_not_filter_natural_language_symbols() {
        ...
        assert!(!hook.contains("pos_shell_syntax_pattern"));
        assert!(!hook.contains("pos_leading_pattern"));
    }
    ...
    fn installing_again_refreshes_an_existing_hook_path() {
        ...
        std::fs::write(
            &rc_path,
            format!("before\n{BEGIN_MARKER}\nsource '/old/pos-hook.zsh'\n{END_MARKER}\nafter\n"),
        )
        .unwrap();
        let hook = temp.path().join("new pos-hook.zsh");
        ...
        assert!(updated.contains("new pos-hook.zsh"));
        assert!(!updated.contains("/old/pos-hook.zsh"));
