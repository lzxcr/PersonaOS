...
fn completion_entries() -> [(&'static str, &'static str); 16] {
    [
        ...
        (
            "remove-shell-hook",
            t(
                "Remove installed PersonaOS shell hooks",
        ...
        (
            "update-default-kb",
            t("Update the default knowledge base", "更新 PersonaOS 默认知识库"),
        ),
        (
            "memory",
...
pub fn hook() -> String {
    ...
    for (command, description) in completion_entries() {
        output.push_str(&format!(
            "complete -c pos -n __fish_use_subcommand -f -a {command} -d '{description}'\n"
        ));
    }
    output.push('\n');
    output.push_str(
        r#"function __pos_paste
    set -l output (pos --clipboard-paste 2>/dev/null)
    if test $status -eq 0; and test -n "$output"
        if not set -q __pos_image_counter
            set -g __pos_image_counter 0
        end
        set __pos_image_counter (math $__pos_image_counter + 1)
        set output (string replace "Image 1" "Image $__pos_image_counter" -- $output)
...
function __pos_insert_newline
    commandline -f expand-abbr
    commandline -i \n
end

bind ctrl-j __pos_insert_newline
bind \cj __pos_insert_newline
bind -M insert ctrl-j __pos_insert_newline
bind -M insert \cj __pos_insert_newline

function __pos_wrap_fish_prompt
    functions -q __pos_original_fish_prompt; and return
    functions -q fish_prompt; or fish_prompt >/dev/null 2>/dev/null
    functions -q fish_prompt; or return

    functions -c fish_prompt __pos_original_fish_prompt
    function fish_prompt
        if set -q __pos_pending_buffer
            printf '\e[?25l'
        end
        __pos_original_fish_prompt
    end
end

function __pos_replay_buffer
    set -l buffer $argv[1]
...
function __pos_restore_cursor
    printf '\e[?25h'
    set -e __pos_cursor_hidden
end

function __pos_on_prompt --on-event fish_prompt
    set -q __pos_pending_buffer; or return

    set -l buffer $__pos_pending_buffer
    set -e __pos_pending_buffer
    set -e __pos_image_counter

    trap __pos_restore_cursor INT TERM EXIT
    __pos_replay_buffer "$buffer"
    printf '\n'
    printf '%s' "$buffer" | pos --shell-intercept --shell fish --stdin
    set -l pos_status $status
    trap - INT TERM EXIT
    __pos_restore_cursor
    return $pos_status
end

function __pos_execute_or_continue
...
function __pos_buffer_is_multiline
    test (string split \n -- "$argv[1]" | count) -gt 1
end

function __pos_first_command
...
function __pos_accept_line
    ...
    if test -z "$trimmed"
        __pos_execute_or_continue
        return
    end

    if not __pos_buffer_is_multiline "$buffer"
        __pos_execute_or_continue
        return
    end

    set -l first_command (__pos_first_command "$buffer")
    ...
    printf '%s' "$buffer" | pos --shell-classify --shell fish --stdin 2>/dev/null
    ...
    set -e __pos_image_counter
    __pos_wrap_fish_prompt
    set -g __pos_cursor_hidden 1
    history append -- "$buffer"
    set -g __pos_pending_buffer "$buffer"
    commandline -b -- ""
    printf '\e[?25l'
    commandline -f execute
end

bind enter __pos_accept_line
bind \r __pos_accept_line
bind -M insert enter __pos_accept_line
bind -M insert \r __pos_accept_line
...
function fish_command_not_found
    ...
    if test -n "$current_line"; and not string match -qr '[\n\r]' -- "$current_line"
        set -l top_command (__pos_first_command "$current_line")
        if test -z "$top_command"; or not type -q -- "$top_command"
            printf '\n'
            printf '%s' "$current_line" | pos --shell-intercept --shell fish --stdin 2>/dev/null
            return 127
        end
    end
    ...
    pos --shell-intercept --shell fish -- $command 2>/dev/null
    return 127
end
"#,
    );
...
pub fn install(paths: &PersonaPaths) -> Result<()> {
...
pub fn uninstall(paths: &PersonaPaths) -> Result<bool> {
    ...
    if removed {
        println!(
            "{}: fish",
            t("removed PersonaOS shell hook", "已移除 PersonaOS shell hook")
        );
    }
    Ok(removed)
}
...
mod tests {
    ...
    fn fish_hook_defines_command_not_found_handler() {
        ...
        assert!(hook.contains("printf '%s' \"$current_line\" | pos --shell-intercept"));
    ...
    fn fish_hook_defines_curated_top_level_completions() {
        ...
        let completion_lines = hook
            .lines()
            .filter(|line| line.starts_with("complete -c pos "))
            .collect::<Vec<_>>();
    ...
    fn fish_hook_defines_paste_binding() {
        let hook = hook();
        assert!(hook.contains("__pos_paste"));
        assert!(hook.contains("bind \\cv __pos_paste"));
        assert!(hook.contains("pos --clipboard-paste"));
    }

    #[test]
    fn fish_hook_defines_enter_binding() {
        let hook = hook();
        assert!(hook.contains("__pos_accept_line"));
        assert!(hook.contains("__pos_wrap_fish_prompt"));
        assert!(hook.contains("functions -c fish_prompt __pos_original_fish_prompt"));
        assert!(hook.contains("if set -q __pos_pending_buffer"));
        assert!(hook.contains("__pos_replay_buffer"));
        assert!(hook.contains("__pos_on_prompt --on-event fish_prompt"));
        assert!(hook.contains("__pos_replay_buffer \"$buffer\"\n    printf '\\n'"));
        ...
        assert!(hook.contains("set -g __pos_cursor_hidden 1"));
        assert!(hook.contains("set -e __pos_cursor_hidden"));
        assert!(hook.contains("return $pos_status"));
        assert!(hook.contains("__pos_execute_or_continue"));
        assert!(hook.contains("__pos_buffer_is_multiline"));
        assert!(hook.contains("test (string split \\n -- \"$argv[1]\" | count) -gt 1"));
        assert!(hook.contains("__pos_first_command"));
        assert!(hook.contains("commandline --input=\"$argv[1]\" --tokens-expanded"));
        assert!(hook.contains("type -q -- \"$first_command\""));
        assert!(hook.contains("set -g __pos_pending_buffer \"$buffer\""));
        ...
        assert!(!hook.contains("__pos_multiline_has_unknown_command"));
        assert!(hook.contains("--shell-classify --shell fish --stdin"));
        assert!(hook.contains("--shell-intercept --shell fish --stdin"));
        assert!(hook.contains("bind enter __pos_accept_line"));
        assert!(hook.contains("bind \\r __pos_accept_line"));
        assert!(hook.contains("bind ctrl-j __pos_insert_newline"));
        assert!(hook.contains("bind -M insert enter __pos_accept_line"));
        assert!(hook.contains("bind -M insert ctrl-j __pos_insert_newline"));
    }

    #[test]
    fn fish_hook_resets_image_counter_on_command_not_found() {
        let hook = hook();
        assert!(hook.contains("set -e __pos_image_counter"));
    }
