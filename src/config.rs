...
fn default_real_context_moderation_keywords() -> Vec<String> {
    ...
    // Keep this self-contained so PersonaOS never reads another application's files.
    const KEYWORDS: &[&str] = &[
        "3p",
        "4p",
        "64",
...
impl Default for PlatformPersonaOverride {
    fn default() -> Self {
        Self::Inherit
    }
}
...
// ── Serde (tag = "mode", internally tagged) ────────────────────────────
//
        ...
        S: serde::Serializer,
    {
        ...
        match self {
            Self::Inherit => {
                ...
                map.serialize_entry("mode", "inherit")?;
                map.end()
            }
            Self::Builtin { name } => {
                let mut map = serializer.serialize_map(Some(2))?;
                map.serialize_entry("mode", "builtin")?;
                map.serialize_entry("name", name)?;
                map.end()
            }
            Self::Custom { name } => {
                let mut map = serializer.serialize_map(Some(2))?;
                map.serialize_entry("mode", "custom")?;
                map.serialize_entry("name", name)?;
                map.end()
            }
        }
...
impl<'de> Deserialize<'de> for PlatformPersonaOverride {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        struct PersonaVisitor;

        impl<'de> Visitor<'de> for PersonaVisitor {
            type Value = PlatformPersonaOverride;

            fn expecting(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
                formatter.write_str("a persona mode object with \"mode\" field")
            }
                ...
                A: MapAccess<'de>,
            {
                ...
                match mode.as_str() {
                    "inherit" => Ok(PlatformPersonaOverride::Inherit),
                    "builtin" => Ok(PlatformPersonaOverride::Builtin {
                        name: name.unwrap_or_default(),
                    }),
                    "custom" => Ok(PlatformPersonaOverride::Custom {
                        name: name.unwrap_or_default(),
                    }),
                    other => Err(Error::unknown_variant(
                        other,
                        &["inherit", "builtin", "custom"],
                    )),
                }
            }
        }

        deserializer.deserialize_map(PersonaVisitor)
    }
}
        ...
        if persona.trim().is_empty() {
            if let Ok(default) = crate::prompts::default_builtin_persona() {
                let default_name = default
                    .meme_library
                    .unwrap_or(default.canonical_name);
                return self
                    .persona_libraries
                    .get("default")
                    .cloned()
...
impl ProviderConfig {
    ...
    pub fn resolved_api_keys(&self, _paths: &PersonaPaths) -> Result<Vec<ResolvedProviderKey>> {
...
impl AppConfig {
    pub fn display_language_hint(paths: &PersonaPaths) -> Option<String> {
    ...
    pub fn load(paths: &PersonaPaths) -> Result<Self> {
    ...
    pub fn load_or_default(paths: &PersonaPaths) -> Result<Self> {
    ...
    pub fn init_files(paths: &PersonaPaths) -> Result<()> {
        paths.create_dirs()?;
        if !paths.config_file.exists() {
    ...
    pub fn save(&self, paths: &PersonaPaths) -> Result<()> {
    ...
    fn normalize_managed_output_paths(&mut self, paths: &PersonaPaths) {
        ...
        remap_managed_output_dir(
            ...
            &[documents.join("PersonaOS"), documents.join("pos")],
            &paths.data_dir.join("documents"),
            base.home_dir(),
        );
        remap_managed_output_dir(
            &mut self.plugins.image_generation.output_dir,
            &[pictures.join("pos"), pictures.join("PersonaOS")],
    ...
    pub fn apply_qq_conversation_persona(
        ...
        match persona {
            ...
            PlatformPersonaOverride::Builtin { name } => {
                let is_default = crate::prompts::default_builtin_persona()
                    .map(|default| name == default.canonical_name)
                    .unwrap_or(false);
                if is_default {
                    // Default builtin (PersonaOS) → empty active_persona (legacy behaviour).
                    self.prompt.active_persona.clear();
                } else {
                    self.prompt.active_persona =
                        crate::prompts::builtin_persona_file(&name);
                }
            }
    ...
    pub fn system_prompt(&self, paths: &PersonaPaths) -> Result<String> {
        self.system_prompt_for(paths, PromptAudience::Owner)
    }

    pub fn system_prompt_for(&self, paths: &PersonaPaths, audience: PromptAudience) -> Result<String> {
    ...
    pub fn base_system_prompt(&self, paths: &PersonaPaths) -> Result<String> {
        let active = self.prompt.active_persona.trim();
        if active.is_empty() {
            return crate::prompts::default_system_prompt();
        }
        // If a disk file exists for the active persona, use it.
        let path = self.persona_path(paths, active);
        if path.exists() {
        ...
        if let Some(persona) = crate::prompts::builtin_persona(canonical) {
            return crate::prompts::builtin_persona_prompt(persona.canonical_name);
        }
        // Not a builtin and no disk file: fall through to legacy custom prompt
        // or default builtin.
    ...
    pub fn custom_system_prompt(&self, paths: &PersonaPaths) -> Result<String> {
    ...
    pub fn identities_dir_path(&self, paths: &PersonaPaths) -> PathBuf {
    ...
    pub fn persona_path(&self, paths: &PersonaPaths, name: &str) -> PathBuf {
        self.prompts_dir_path(paths).join(name)
    }

    pub fn validate_persona_files(&self, paths: &PersonaPaths) -> Result<()> {
        if self
            .prompt
            .active_persona
            .trim()
    ...
    pub fn identity_path(&self, paths: &PersonaPaths, name: &str) -> PathBuf {
        self.identities_dir_path(paths).join(name)
    }

    pub fn persona_memory_data_dir(&self, paths: &PersonaPaths, persona: &str) -> PathBuf {
        paths
            .data_dir
            .join("personas")
    ...
    pub fn persona_memory_state_dir(&self, paths: &PersonaPaths, persona: &str) -> PathBuf {
        paths
            .state_dir
            .join("personas")
            .join(persona_scope_name(persona))
    ...
    pub fn persona_skills_dir(&self, paths: &PersonaPaths, persona: &str) -> PathBuf {
        paths
            .skills_dir
            .join("personas")
            .join(persona_scope_name(persona))
    ...
    pub fn active_persona_memory_data_dir(&self, paths: &PersonaPaths) -> PathBuf {
        self.persona_memory_data_dir(paths, self.prompt.active_persona.trim())
    }

    pub fn active_persona_memory_state_dir(&self, paths: &PersonaPaths) -> PathBuf {
        self.persona_memory_state_dir(paths, self.prompt.active_persona.trim())
    }

    pub fn active_persona_skills_dir(&self, paths: &PersonaPaths) -> PathBuf {
        self.persona_skills_dir(paths, self.prompt.active_persona.trim())
    }

    pub fn active_persona_prompt(&self, paths: &PersonaPaths) -> Result<String> {
    ...
    pub fn user_identity_prompt(&self, paths: &PersonaPaths) -> Result<String> {
    ...
    pub fn system_prompt_path(&self, paths: &PersonaPaths) -> PathBuf {
        let value = self
            .system_prompt_file
            .as_deref()
            .map(str::trim)
...
fn migrated_resource_path(paths: &PersonaPaths, value: &str) -> Option<PathBuf> {
    paths.migrated_resource_path(Path::new(value.trim()))
}

fn fallback_resource_file(paths: &PersonaPaths, namespace: &str, file_name: &str) -> PathBuf {
...
fn config_relative_path(paths: &PersonaPaths, value: &str) -> PathBuf {
...
fn default_pos_home() -> PathBuf {
    std::env::var_os("POS_HOME")
        .map(PathBuf::from)
        .or_else(|| directories::BaseDirs::new().map(|dirs| dirs.home_dir().join(".pos")))
        .unwrap_or_else(|| PathBuf::from("~/.pos"))
}
...
mod tests {
    ...
    fn platforms_config_roundtrip_and_default_omission() {
        ...
        assert_eq!(qq.group_chats.trigger_keywords, vec!["PersonaOS"]);
    ...
    fn qq_conversation_persona_override_is_explicit_and_tracks_renames() {
        ...
        config.platforms.qq.conversations[0].persona =
            PlatformPersonaOverride::Builtin {
                name: "test".to_string(),
            };
    ...
    fn mixed_context_window_uses_the_global_default_when_model_metadata_is_missing() {
        ...
        provider.models = vec![
            "pos-known-window-model".to_string(),
            "pos-unknown-window-model".to_string(),
        ];
        ...
        config.providers[0]
            .model_context_window
            .insert("pos-unknown-window-model".to_string(), 128_000);
    ...
    fn display_language_hint_reads_jsonc_without_loading_full_config() {
        ...
        let paths = PersonaPaths {
            ...
            fish_hook_file: temp.path().join("pos.fish"),
            bash_hook_file: temp.path().join("pos.bash"),
            zsh_hook_file: temp.path().join("pos.zsh"),
    ...
    fn meme_library_defaults_follow_persona() {
        ...
        assert_eq!(memes.library_for_persona(""), "test");
        assert_eq!(
    ...
    fn default_prompt_resources_follow_the_data_resource_layout() {
        ...
        let paths = PersonaPaths {
            ...
            fish_hook_file: temp.path().join("fish/pos.fish"),
        ...
        let root = base.home_dir().join(".pos");
        ...
        legacy_absolute.prompt.user_identity_file = base
            .config_dir()
            .join("pos/user-identity.md")
            .display()
            .to_string();
        legacy_absolute.system_prompt_file = Some(
            base.config_dir()
                .join("pos/system-prompt.md")
                .display()
                .to_string(),
        );
        assert_eq!(
