...
impl PersonaPaths {
    pub fn new() -> Result<Self> {
        ...
        let legacy_config_dir = base.config_dir().join("pos");
        let legacy_data_dir = base.data_dir().join("pos");
        let legacy_cache_dir = base.cache_dir().join("pos");
        ...
        let explicit_home = std::env::var_os("POS_HOME").map(PathBuf::from);
        let root_dir = explicit_home
            .clone()
            .unwrap_or_else(|| base.home_dir().join(".pos"));
        ...
        let legacy = LegacyLayout {
            ...
            pictures_dirs: vec![
                legacy_pictures_root.join("pos"),
                legacy_pictures_root.join("PersonaOS"),
            ],
        };
        let next = Layout {
        ...
        let fish_hook_file = base.config_dir().join("fish/conf.d/pos.fish");
        ...
        let system_scripts_dir = PathBuf::from("/usr/share/pos/scripts");
    ...
    /// Returns the root used for PersonaOS-owned, user-authored resources. During
    ...
    pub fn legacy_config_dir(&self) -> Option<PathBuf> {
        ...
        (self.config_dir == base.home_dir().join(".pos/config"))
            .then(|| base.config_dir().join("pos"))
    }
    ...
    pub fn runtime_dir(&self) -> PathBuf {
        match std::env::var_os("XDG_RUNTIME_DIR") {
            Some(runtime_dir) => runtime_dir_for(
                ...
                std::env::var_os("POS_HOME").as_deref().map(Path::new),
            ),
            None => self.state_dir.join("pos"),
        }
    }
...
fn resource_runtime_dir(layout: &Layout) -> PathBuf {
    if cfg!(test) {
        return layout.state_dir.join("pos");
    }
    match std::env::var_os("XDG_RUNTIME_DIR") {
        ...
        None => layout.state_dir.join("pos"),
    }
}
...
fn marker_exists_at(path: &Path, label: &str) -> Result<bool> {
    match fs::symlink_metadata(path) {
        Ok(metadata) if metadata.file_type().is_symlink() => {
            bail!(
                "PersonaOS {label} must not be a symbolic link: {}",
                path.display()
            )
        }
        Ok(metadata) if !metadata.is_file() => {
            bail!("PersonaOS {label} is not a regular file: {}", path.display())
        }
        Ok(_) => Ok(true),
...
fn migrate_resource_layout(layout: &Layout) -> Result<()> {
    if !try_migrate_resource_layout(layout, false)? {
        bail!("PersonaOS resource migration is deferred while another daemon or starter is active");
    }
    Ok(())
}
        ...
        if let Err(error) = atomic_resource_move(&entry.source, &entry.destination) {
            ...
            return match recovery {
                Ok(()) => Err(error).with_context(|| {
                    format!(
                        "migrating PersonaOS resource from {} to {}",
...
fn preflight_resource_entries(layout: &Layout, entries: &[ResourceMigrationEntry]) -> Result<()> {
    ...
    for entry in entries {
        ...
        if source_metadata.file_type().is_symlink() {
            bail!(
                "PersonaOS resource migration refuses symbolic-link source {}",
                entry.source.display()
            );
        }
        ...
        match fs::symlink_metadata(&entry.destination) {
            Ok(_) => bail!(
                "PersonaOS resource migration destination already exists: {}; move or remove it and retry",
                entry.destination.display()
            ),
...
fn ensure_destination_ancestors(layout: &Layout, destination: &Path) -> Result<()> {
    let relative = destination
        ...
        .with_context(|| {
            format!(
                "resource destination escapes PersonaOS data directory: {}",
                destination.display()
            )
        })?;
    ...
    for component in relative.components() {
        ...
        match fs::symlink_metadata(&current) {
            Ok(metadata) if metadata.file_type().is_symlink() => bail!(
                "PersonaOS resource migration refuses symbolic-link destination ancestor {}",
                current.display()
            ),
            Ok(metadata) if !metadata.is_dir() => bail!(
                "PersonaOS resource migration destination ancestor is not a directory: {}",
                current.display()
            ),
            Ok(_) => {}
...
fn ensure_resource_same_filesystem(source: &Path, destination: &Path) -> Result<()> {
    ...
    if source_device != destination_device {
        bail!(
            "PersonaOS resource migration requires source and destination on the same filesystem: {} -> {}",
            source.display(),
            destination.display()
        );
    }
...
fn ensure_resource_targets_do_not_overlap(
    ...
    if entry_exists(&nested_source)? && entry_exists(&child.source)? {
        bail!(
            "PersonaOS resource migration found overlapping sources for {} and {}; remove one duplicate and retry",
...
fn recover_resource_migration(layout: &Layout) -> Result<()> {
    ...
    if journal.moved > journal.entries.len() {
        bail!("invalid PersonaOS resource migration journal: moved count is out of range");
    }
    if let Some(index) = journal.pending {
        if index != journal.moved || index >= journal.entries.len() {
            bail!("invalid PersonaOS resource migration journal: pending index is out of range");
        }
        ...
        match (
            ...
            (true, true) => bail!(
                "cannot recover PersonaOS resource migration because both paths exist: {} and {}",
                entry.source.display(),
                entry.destination.display()
            ),
            (false, false) => bail!(
                "cannot recover PersonaOS resource migration because both paths are missing: {} and {}",
                entry.source.display(),
                entry.destination.display()
            ),
        }
        ...
        match (
            ...
            (false, true) => {
                atomic_resource_move(&entry.destination, &entry.source).with_context(|| {
                    format!(
                        "rolling back PersonaOS resource migration from {} to {}",
                        entry.destination.display(),
                        entry.source.display()
                    )
                })?
...
fn legacy_daemon_is_running_at(legacy: &LegacyLayout, xdg_runtime_dir: Option<&Path>) -> bool {
    ...
    runtime_dirs
        .into_iter()
        .map(|runtime_dir| runtime_dir.join("pos"))
        .any(|runtime_dir| {
            std::os::unix::net::UnixStream::connect(runtime_dir.join("core.sock")).is_ok()
                || runtime_lock_is_held(&runtime_dir.join("core.lock"))
...
fn migrate_legacy_layout(legacy: &LegacyLayout, next: &Layout) -> Result<()> {
    ...
    // guarantees a known conflict does not even create ~/.pos; this pass
    // closes the race with another new PersonaOS process before any data moves.
    ...
    for mapping in &active {
        migrate_entry_unchecked(&mapping.source, &mapping.destination).with_context(|| {
            format!(
                "migrating PersonaOS user files from {} to {}",
    ...
    if had_bash_hook || had_zsh_hook {
        let home = next
            .root_dir
            .parent()
            .context("the PersonaOS home directory has no parent")?;
        ...
        .context("refreshing shell hook paths after PersonaOS directory migration")?;
    }
...
fn existing_mappings(mappings: &[MigrationMapping]) -> Result<Vec<MigrationMapping>> {
    ...
    for mapping in mappings {
        ...
        {
            bail!(
                "PersonaOS directory migration cannot move overlapping paths: {} and {}",
                mapping.source.display(),
                mapping.destination.display()
            );
        }
    ...
    for (index, left) in active.iter().enumerate() {
        for right in active.iter().skip(index + 1) {
            ...
            {
                bail!(
                    "PersonaOS directory migration has overlapping legacy sources: {} and {}",
                    left.source.display(),
...
fn acquire_migration_lock(root: &Path) -> Result<MigrationLease> {
    ...
    if result != 0 {
        return Err(std::io::Error::last_os_error()).context("locking PersonaOS directory migration");
    }
...
fn ensure_existing_directory(path: &Path) -> Result<()> {
    match fs::symlink_metadata(path) {
        Ok(metadata) if metadata.file_type().is_symlink() => bail!(
            "PersonaOS refuses to use a symbolic-link directory: {}",
            path.display()
        ),
        Ok(metadata) if !metadata.is_dir() => bail!(
            "PersonaOS expected a directory but found another file: {}",
            path.display()
        ),
        Ok(_) => Ok(()),
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => Ok(()),
...
fn layout_marker_exists(layout: &Layout) -> Result<bool> {
    ...
    match fs::symlink_metadata(&marker) {
        Ok(metadata) if metadata.file_type().is_symlink() => bail!(
            "PersonaOS layout marker must not be a symbolic link: {}",
            marker.display()
        ),
        Ok(metadata) if !metadata.is_file() => bail!(
            "PersonaOS layout marker is not a regular file: {}",
            marker.display()
        ),
        Ok(_) => Ok(true),
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => Ok(false),
...
fn ensure_supported_entry_tree(path: &Path) -> Result<()> {
    ...
    if metadata.file_type().is_symlink() {
        ...
        if target.is_relative() {
            bail!(
                "PersonaOS directory migration refuses relative symbolic link {}; its target would change after moving",
                path.display()
            );
        }
        return Ok(());
        ...
        if target.is_absolute() {
            for (source, destination) in projections {
                if let Ok(relative) = target.strip_prefix(source) {
                    bail!(
                        "PersonaOS directory migration refuses symbolic link {} because its absolute target moves from {} to {}",
                        path.display(),
                        target.display(),
...
fn projected_source_entry(source_root: &Path, relative: &Path) -> Result<Option<PathBuf>> {
    ...
    for component in relative.components() {
        ...
        if !metadata.is_dir() {
            bail!(
                "PersonaOS directory migration found a projected path conflict at {}",
                current.display()
            );
        }
...
fn ensure_projected_entries_compatible(
    ...
    bail!(
        "PersonaOS directory migration found conflicting legacy entries {} and {} projected to {}",
        left.display(),
        right.display(),
...
fn ensure_no_conflicts(source: &Path, destination: &Path) -> Result<()> {
    ...
    match fs::symlink_metadata(destination) {
        Ok(destination_meta) => {
            ...
            bail!(
                "PersonaOS directory migration found conflicting entries: {} and {}; move or rename one of them and retry",
                source.display(),
                destination.display()
            );
        }
...
fn migrate_entry_unchecked(source: &Path, destination: &Path) -> Result<()> {
    ...
    match fs::symlink_metadata(destination) {
        Ok(destination_meta) => {
            ...
            bail!(
                "PersonaOS directory migration found a conflict that appeared after preflight: {} and {}",
                source.display(),
                destination.display()
            );
        }
...
mod tests {
    ...
    fn test_layouts(root: &Path) -> (LegacyLayout, Layout) {
        (
            LegacyLayout {
                ...
                documents_dir: root.join("Documents/PersonaOS"),
                pictures_dirs: vec![root.join("Pictures/pos"), root.join("Pictures/PersonaOS")],
            },
            Layout {
                root_dir: root.join(".pos"),
                config_dir: root.join(".pos/config"),
                data_dir: root.join(".pos/data"),
                cache_dir: root.join(".pos/cache"),
                state_dir: root.join(".pos/state"),
            },
        )
    }
    ...
    fn resource_path_remapping_includes_the_legacy_xdg_config_root() {
        ...
        let paths = PersonaPaths {
            ...
            fish_hook_file: base.config_dir().join("fish/conf.d/pos.fish"),
            bash_hook_file: root.join("config/shell/bash-hook.sh"),
            zsh_hook_file: root.join("config/shell/zsh-hook.zsh"),
            scripts_dir: root.join("data/scripts"),
            system_scripts_dir: PathBuf::from("/usr/share/pos/scripts"),
        };
        assert_eq!(
            paths.migrated_resource_path(&base.config_dir().join("pos/prompts/team")),
    ...
    fn legacy_layout_stays_put_while_the_core_lock_is_held() {
        ...
        let runtime_dir = legacy.state_dir.join("pos");
    ...
    fn resource_migration_holds_runtime_exclusion_through_commit() {
        ...
        let runtime_dir = layout.state_dir.join("pos");
        fs::create_dir_all(&runtime_dir).unwrap();
