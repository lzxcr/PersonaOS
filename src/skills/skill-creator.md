---
name: skill-creator
description: Create or safely update PersonaOS skills. Use when the user asks to create, author, improve, or modify a reusable skill, SKILL.md workflow, or skill resources.
compatibility: PersonaOS built-in skill authoring workflow
---
...
4. Use `create_skill` for a new skill or `update_skill` for an existing skill. Use the returned absolute `skill_file` and `skill_dir`; never guess a PersonaOS home path.
...
Optional standard fields are `license`, `compatibility`, `metadata`, and `allowed-tools`. `allowed-tools` is compatibility metadata only and never grants PersonaOS permissions.
