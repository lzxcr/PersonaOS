(() => {
  ...
  const state = {
    ...
    persona: {
      name: "PersonaOS",
      avatar_url: "/assets/pos-logo.png",
      board_image_url: "/assets/pos-wallpaper.png",
  ...
  function setTheme(theme, persist = true) {
    ...
    if (persist) safeStorageSet("pos.web.theme", selected);
  }
  ...
  function setColorScheme(scheme, persist = true) {
    ...
    if (persist) safeStorageSet("pos.web.colorScheme", requested);
  }
  ...
  async function probeMatugenTheme() {
    ...
    setColorScheme(safeStorageGet("pos.web.colorScheme") || (state.matugenAvailable ? "matugen" : "madobe"), false);
  }
  ...
  function setChatFontSize(size, persist = true) {
    ...
    if (persist) safeStorageSet("pos.web.chatFontSize", selected);
  }
  ...
  function setReasoningExpanded(value, persist = true) {
    ...
    if (persist) safeStorageSet("pos.web.reasoningExpanded", String(state.reasoningExpanded));
  }
  ...
  function setToolExpanded(value, persist = true) {
    ...
    if (persist) safeStorageSet("pos.web.toolExpanded", String(state.toolExpanded));
  }
  ...
  function setMode(mode, persist = true, animate = false) {
    ...
    if (persist) safeStorageSet("pos.web.mode", selected);
  }
  ...
  function setSidebarCollapsed(collapsed, { automatic = false } = {}) {
    ...
    if (!automatic) safeStorageSet("pos.web.sidebarCollapsed", String(state.sidebarCollapsed));
    syncArtifactLayout?.();
  }
  ...
  function normalizePersona(value) {
    const name = String(value?.name || "").trim() || "PersonaOS";
  ...
  function renderPromptCollection(kind, titleText, activePath) {
    ...
    defaultOption.value = "";
    defaultOption.textContent = kind === "personas" ? "PersonaOS 默认人格" : "不使用用户身份";
  ...
  async function apiRequest(path, options = {}) {
    ...
    try {
      ...
      throw new ApiError("无法连接 PersonaOS WebUI", 0);
    }
  ...
  function uploadComposerAttachment(item) {
    ...
    request.setRequestHeader("X-PersonaOS-Filename", encodeURIComponent(item.file.name));
  ...
  function createConversationMedia(asset, { eager = false } = {}) {
    ...
    const alt = String(source.alt || "").trim() || "PersonaOS 生成的图片";
   ...
   * 默认展开/收起由本地偏好 pos.web.reasoningExpanded 决定,与 summary/full 无关。
   */
  ...
  function showBlockedState(unauthorized, message = "") {
    ...
    elements.blockedTitle.textContent = unauthorized ? "登录 PersonaOS" : "无法载入 PersonaOS WebUI";
  ...
  function bindEvents() {
    ...
    elements.artifactResizeHandle.addEventListener("pointerdown", (event) => {
      ...
      const finish = () => {
        ...
        safeStorageSet("pos.web.artifactWidthRatio.v2", String(state.artifactWidthRatio));
  ...
  function initialize() {
    renderIconSlots();
    setTheme(safeStorageGet("pos.web.theme") || "graphite", false);
    const storedScheme = safeStorageGet("pos.web.colorScheme");
    if (storedScheme) setColorScheme(storedScheme, false);
    probeMatugenTheme();
    setChatFontSize(safeStorageGet("pos.web.chatFontSize") || "15px", false);
    setReasoningExpanded(safeStorageGet("pos.web.reasoningExpanded") === "true", false);
    setToolExpanded(safeStorageGet("pos.web.toolExpanded") === "true", false);
    setMode(safeStorageGet("pos.web.mode") || "normal", false);
    const artifactRatio = Number(safeStorageGet("pos.web.artifactWidthRatio.v2"));
    if (Number.isFinite(artifactRatio) && artifactRatio >= 0.25 && artifactRatio <= 0.9) {
      state.artifactWidthRatio = artifactRatio;
    }
    setSidebarCollapsed(safeStorageGet("pos.web.sidebarCollapsed") === "true");
    syncArtifactLayout();
