# PersonaOS WebUI assets

These static assets are embedded into the PersonaOS daemon at build time. Run the local WebUI with:

```sh
cargo run --bin pos -- web
```

The command starts the PersonaOS daemon (the same `pos` executable re-run in daemon mode) when needed, prints the access URLs, and exits. Use `pos daemon status` or `pos daemon stop` to inspect or stop the daemon. WebUI listens on all local network interfaces by default. Password protection is optional:

```sh
cargo run --bin pos -- web -p secret
cargo run --bin pos -- web -p
cargo run --bin pos -- web --password-file /path/to/password.txt
```
...
Two built-in themes derive from the PersonaOS logo:
...
The 404 when no override exists is harmless. Serving `~/.pos/config/webui-theme.css`
