...
fn pos_command(binary: &Path, home: &Path, runtime: &Path) -> Command {
...
impl Drop for DaemonGuard {
    fn drop(&mut self) {
        let _ = pos_command(&self.binary, &self.home, &self.runtime)
            .args(["daemon", "stop"])
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .status();
...
fn run(binary: &Path, home: &Path, runtime: &Path, args: &[&str]) -> Output {
    pos_command(binary, home, runtime)
        .args(args)
        .output()
        .unwrap()
}
...
fn reload_fails_without_starting_a_daemon() {
    ...
    let binary = PathBuf::from(env!("CARGO_BIN_EXE_pos"));

    assert!(run(&binary, &home, &runtime, &["init"]).status.success());
    let reload = run(&binary, &home, &runtime, &["reload"]);
    assert!(!reload.status.success());
    assert!(String::from_utf8_lossy(&reload.stderr).contains("PersonaOS daemon is not running"));

    let status = run(&binary, &home, &runtime, &["daemon", "status"]);
    assert!(status.status.success());
    assert!(String::from_utf8_lossy(&status.stdout).contains("PersonaOS daemon: stopped"));
}
...
fn reload_applies_config_without_restarting_daemon() {
    ...
    assert!(init.status.success(), "pos init failed: {init:?}");

    let child = pos_command(&binary, &home, &runtime)
    ...
    loop {
        ...
        if status.status.success()
            && String::from_utf8_lossy(&status.stdout).contains("PersonaOS daemon: running")
        {
            break;
        }
    ...
    let reload = run(&binary, &home, &runtime, &["reload"]);
    assert!(reload.status.success(), "pos reload failed: {reload:?}");
